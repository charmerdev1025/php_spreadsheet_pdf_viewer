<?php
  session_start();
  include 'config.php';
  $sectors_query = "select sector from finances where sector != '' group by sector";
  $sector_limit_query = "select sector from finances where sector != '' group by sector limit 1";
  $sectors = $conn->query($sectors_query);
  $sector_result = $conn->query($sector_limit_query);
  $sector = $sector_result->fetch_array()["sector"];
  if(isset($_GET['keyword'])) {
    $sector = $_GET['keyword'];
  }
  $finances_sector_query = "select * from finances where sector='".$sector."'";
  $finances_sector = $conn->query($finances_sector_query);
  $keyword = "";
  if(isset($sector)) {
    $keyword = $sector;
  }
  if(!isset($finances_sector)) $finances_sector = array();
?>
<!DOCTYPE html>
<html>
...
<body class="g-sidenav-show bg-gray-100">
  ...
  <aside
    ...
    <div class="collapse navbar-collapse  w-auto h-auto" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="sector.php">
            <div class="icon icon-shape icon-sm text-center  me-2 d-flex align-items-center justify-content-center">
      ...
      <div class="row mt-4">
        <div class="col-12">
          <div class="card card-body">
            <div class="row">
              <div class="col-6">
                <label class="form-label mt-4">Please choose sector</label>
                <select class="form-control sector" name="choices-sector" id="choices-sector">
              <?php
                foreach ($sectors as $sector) {
              ?>
                  <option value="<?php echo $sector["sector"];?>" <?php if($sector["sector"] == $keyword) echo " selected";?>><?php echo $sector["sector"];?></option>
              <?php
                }
                ?>
                </select>
          ...
          <div class="card mt-4">
            <div class="table-responsive">
              <table class="table table-flush" id="datatable-search">
                ...
                while($finance = $finances_sector->fetch_assoc()) {
                  $no++;
                ?>
                  <tr>
  ...
  <script>
    ...
    if (document.getElementById('choices-sector')) {
      var sectors = document.getElementById('choices-sector');
      const example = new Choices(sectors);
    }
    ...
    $(".sector").change(function() {
      let sector = $(this).val();
      let url = "sector.php?keyword=" + sector;
